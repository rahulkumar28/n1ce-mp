...
contract N1ceMP is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint saleCommision = 5;
    //resales comission for marketplace for any resell
    uint resellCommision = 2;
    //enum to restrict the type of tokens
    enum TokenType { METAVERSE, AVATAR, GAMES, MOVIES, EVENT }
    ...
    function setSaleCommision(uint comission) public onlyOwner{
    ...
    function setReSellCommision(uint comission) public onlyOwner{
    ...
    function updateItemPrice(uint256 itemId,uint256 newPrice)  public {
        ...
        require(msg.sender == idToMarketItem[itemId].owner,"Not an Owner of the Item");
        //check price should be more than 0
    ...
    */
    function purchaseNCT(uint256 itemId)
        public
        payable
        nonReentrant
    {
        //check if not sending to yoruself
        require(msg.sender!=idToMarketItem[itemId].owner,"Sending to Yourself not allowed");

        //check if item avaiable for sale (1st time or on resale)
        bool isAvailable = (!idToMarketItem[itemId].sold||idToMarketItem[itemId].onResale)? true:false;
        ...
        bool isSupplyAvailable = (idToMarketItem[itemId].count < idToMarketItem[itemId].totalSupply) ? true:false;
        ...
        uint256 comission4Sale= (idToMarketItem[itemId].sold)?(saleCommision*price/100):(resellCommision*price/100);
        //calculate royalty
        ...
        //used safe openzepplin sendvalue function
        Address.sendValue(payable(ownerOf(itemId)),(msg.value - royalty4sale - comission4Sale));
        //old implementation commented
